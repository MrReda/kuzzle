...
class GarbageCollector {
  ...
  run () {
    ...
    return this.kuzzle.pluginsManager.trigger('gc:start')
          ...
          .each(icpair => {
            return this.clearCollection(icpair.index, icpair.collection)
              .then(deleted => {
                if (deleted > 0) {
                  total += deleted;
                  this.kuzzle.pluginsManager.trigger('log:info', `[GC] ${icpair.index}/${icpair.collection}: trashcan emptied (${deleted} documents deleted)`);
                }
      ...
      .then(() => {
        if (total.length) {
          this.kuzzle.pluginsManager.trigger('log:info', `[GC] Finished: ${total} documents deleted in all trashcans`);
        }

        this.kuzzle.pluginsManager.trigger('gc:end', {deleted: total});
  ...
  clearCollection(index, collection) {
    ...
    return this.kuzzle.services.list.storageEngine.deleteByQuery(request)
