...
class ElasticSearch extends Service {
  ...
  init() {
    ...
    return Bluebird.resolve(this.client.info()
      .then(response => {
        ...
        return this.client.getScript({id: '_kuzzle_info_delete', lang: 'painless'})
          .then(() => this)
          .catch(error => {
            ...
            return this.client.putScript({
              id: '_kuzzle_info_delete',
              lang: 'painless',
              body: {
                script: `if (ctx._source._kuzzle_info == null) {
                  ctx._source._kuzzle_info = ['active': false, 'deletedAt': params.now];
                }
                else {
                  ctx._source._kuzzle_info.active = false;
                  ctx._source._kuzzle_info.deletedAt = params.now;
                }`
              }
            })
              .then(() => this);
  ...
  deleteByQuery(request) {
    ...
    let
      esMethod = req => this.client.updateByQuery(req),
      responseMember = 'updated';
    ...
    if (request.input.args.bypassTrash) {
      // *real* deletion
      esMethod = req => this.client.deleteByQuery(req);
      responseMember = 'deleted';
    }
    else {
      esRequest.body.script = {
        lang: 'painless',
        stored: '_kuzzle_info_delete',
        params: {now: Date.now()}
      };
    }
    esRequest.conflicts = request.input.args.conflicts || 'proceed';

    if (request.input.args.refresh) {
      esRequest.refresh = true;
    }

    // bypass default request timeout (30s)
    esRequest.requestTimeout = 0;

    let deleted = 0;
    let conflicts = 0;
    const deleteBatch =(req, attempts = 0) => esMethod(req)
      .then(response => {
        deleted += response[responseMember];

        if (response.version_conflicts === 0) {
          return deleted;
        }

        // version conflicts detected => retry
        if (attempts < this.config.defaults.onUpdateConflictRetries) {
          conflicts += response.version_conflicts;
          return deleteBatch(req, attempts + 1);
        }
        throw new ExternalServiceError(`Version conflicts error (${conflicts} encountered)`);
      })
      .catch(error => {
        // conflict error
        if (error.statusCode === 409 && attempts < this.config.defaults.onUpdateConflictRetries) {
          return deleteBatch(req, attempts + 1);
        }
        throw error;
      });

    // refresh is needed here to prevent versions conflicts and is acceptable in a delete by query context
    return this.client.indices.refresh({index: esRequest.index})
      .then(() => deleteBatch(esRequest))
      .then(response => this.refreshIndexIfNeeded(esRequest, {deleted: response}))
      .catch(error => Bluebird.reject(this.esWrapper.formatESError(error)));
  }
