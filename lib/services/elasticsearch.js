...
class ElasticSearch extends Service {
  ...
  deleteByQuery(request) {
    ...
    if (request.input.args.bypassTrash) {
      ...
      esRequest.conflicts = request.input.args.conflicts || 'abort';
    }
    ...
    return this.client.indices.refresh({index: esRequest.index})
      .then(() => esMethod(esRequest))
      .then(response => {
        this.refreshIndexIfNeeded(esRequest, response);
        return {deleted: response[responseMember]};
      })
      .catch(error => Bluebird.reject(this.esWrapper.formatESError(error)));
  }
